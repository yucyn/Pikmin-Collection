...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const modalBeenThereBtn = document.getElementById("modalBeenThereBtn"); // V38: 去過按鈕
  const beenThereCount = document.getElementById("beenThereCount");
  ...
  `;
    ...
    presenceColl.onSnapshot((snapshot) => {
      ...
      // 如果已經讀取到人數，確保統計容器是顯示的 (如果是在管理員模式下)
  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      const locationText = isV37Locked ? "🍄解鎖蘑菇" : item.locationText;
      ...
      modalCardLocation.innerHTML = `${item.category || "全球"}｜${displayLocation}｜${isOwnedByCurrentUser(item) ? "我的明信片" : "公開明信片"}${noteSuffix}`;
      ...
      // 更新去過的人
      if (modalBeenThereBtn && beenThereCount) {
        const beenThereBy = Array.isArray(item.beenThereBy) ? item.beenThereBy : [];
        const hasBeenThere = beenThereBy.includes(firebase.auth().currentUser?.uid);
        modalBeenThereBtn.classList.toggle("active", hasBeenThere);
        beenThereCount.textContent = formatLikeCount(item.beenThereCount || 0);
      }
        
      const modalLikeBtn = document.getElementById("modalLikeBtn");
      if (modalLikeBtn) {
        const isLiked = typeof isLikedByCurrentUser === "function" ? isLikedByCurrentUser(item) : false;
        modalLikeBtn.innerHTML = isLiked ? "❤️" : "🤍";
        modalLikeBtn.classList.toggle("active", isLiked);
        modalLikeBtn.onclick = async (e) => {
          e.stopPropagation();
          if (typeof togglePostcardLike === "function") {
            await togglePostcardLike(item.id);
            const updatedItem = getPostcardById(item.id);
            if (updatedItem) {
              const nowLiked = isLikedByCurrentUser(updatedItem);
              modalLikeBtn.innerHTML = nowLiked ? "❤️" : "🤍";
              modalLikeBtn.classList.toggle("active", nowLiked);
            }
          }
        };
      }

      if (modalBeenThereBtn) {
        modalBeenThereBtn.onclick = async (e) => {
          e.stopPropagation();
          const uid = typeof getCurrentUserId === "function" ? getCurrentUserId() : null;
          await togglePostcardBeenThere(item.id);
          
          // 為了即時回饋，我們先手動切換 UI 狀態
          const isNowActive = !modalBeenThereBtn.classList.contains("active");
          modalBeenThereBtn.classList.toggle("active", isNowActive);
          const currentVal = parseInt(beenThereCount.textContent) || 0;
          beenThereCount.textContent = isNowActive ? currentVal + 1 : Math.max(0, currentVal - 1);

          // 等待快取更新後再次確認 (可選)
          setTimeout(() => {
            const updatedItem = getPostcardById(item.id);
            if (updatedItem) {
              const nextBeenThereBy = Array.isArray(updatedItem.beenThereBy) ? updatedItem.beenThereBy : [];
              modalBeenThereBtn.classList.toggle("active", uid && nextBeenThereBy.includes(uid));
              beenThereCount.textContent = formatLikeCount(updatedItem.beenThereCount || 0);
            }
          }, 1000);
        };
      }
    ...
    modal.innerHTML = `
      ...
      <form id="editForm" class="edit-modal-panel">
        ...
        <div class="field-full v37-edit-admin-unlock" id="v37EditAdminUnlockBlock" style="display:none;">
          ...
          <label class="v37-edit-check"><input type="checkbox" id="editRequirePlayerUnlock" /> 此卡須完成玩家登入後才可查看圖片與 GPS（可加鎖非蘑菇卡片）</label>
          <label class="v37-edit-check"><input type="checkbox" id="editPublicUnlock" /> 公開顯示（覆蓋蘑菇預設鎖定）</label>
        </div>
  ...
  function handleEditClick(item) {
    ...
    const editRequireUnlock = document.getElementById("editRequirePlayerUnlock");
    const editPublicUnlock = document.getElementById("editPublicUnlock");
    const showAdminUnlock = Boolean(window.PikminAuthGate && typeof window.PikminAuthGate.isFirebaseAdmin === "function" && window.PikminAuthGate.isFirebaseAdmin());
    if (adminUnlockBlock) adminUnlockBlock.style.display = showAdminUnlock ? "block" : "none";
    if (editRequireUnlock) editRequireUnlock.checked = Boolean(item.requirePlayerUnlock);
    if (editPublicUnlock) editPublicUnlock.checked = Boolean(item.publicUnlock);
    ...
    async function submitEditChanges() {
      ...
      try {
        ...
        if (window.PikminAuthGate && typeof window.PikminAuthGate.isFirebaseAdmin === "function" && window.PikminAuthGate.isFirebaseAdmin()) {
          const reqEl = document.getElementById("editRequirePlayerUnlock");
          const pubEl = document.getElementById("editPublicUnlock");
          const rarEl = document.getElementById("editRarityInput");
          if (reqEl) changes.requirePlayerUnlock = Boolean(reqEl.checked);
          if (pubEl) changes.publicUnlock = Boolean(pubEl.checked);
          if (rarEl) changes.rarity = rarEl.value;
        }
  ...
  function updateSidebarCardCount() {
    ...
    try {
      ...
      if (todayEl) {
        ...
        const todayCount = cards.filter(c => c.createdAt && c.createdAt.startsWith(localISODate)).length;
