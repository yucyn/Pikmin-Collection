...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      try {
        isFirebaseAdmin = !!(window.PikminAuthGate && typeof window.PikminAuthGate.isFirebaseAdmin === "function" && window.PikminAuthGate.isFirebaseAdmin());
      } catch (e) { console.warn("Admin check failed", e); }
      
      const isOwner = typeof isOwnedByCurrentUser === "function" && isOwnedByCurrentUser(item);
      // 放寬權限：匿名擁有者也可以編輯自己的卡片
      const canManage = isFirebaseAdmin || isOwner;
  ...
  function handleEditClick(item) {
    ...
    if (saveBtn) {
      ...
      const handleSaveTrigger = async (e) => {
        ...
        // 手機版優化：儲存前強制讓所有輸入框失去焦點，確保輸入法內容已同步
        const activeEl = document.activeElement;
        if (activeEl && (activeEl.tagName === "INPUT" || activeEl.tagName === "TEXTAREA" || activeEl.tagName === "SELECT")) {
          activeEl.blur();
        }
        
        if (isSavingEdit || (saveBtn && saveBtn.disabled)) return;
        ...
        console.log("Save trigger fired (Edit Mode)");
      ...
      // 手機版優化：統一使用 onclick 即可，移除 ontouchend 避免雙重觸發或部分瀏覽器衝突
      saveBtn.onclick = handleSaveTrigger;
      if (saveBtn.ontouchend) saveBtn.ontouchend = null;
    }
