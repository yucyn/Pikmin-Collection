...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initAppFeatures() {
        ...
        if (mlRes.ok) {
          ...
          if (d && (d.title || d.image)) {
            return {
              ...
              image: (d.image && typeof d.image === 'object') ? d.image.url : (d.image || "")
            };
          }
        }
      } catch (e) { console.error("Microlink fallback failed:", e); }
    ...
    async function renderBulletinWithPreview(text) {
      ...
      function applyFormatting(raw) {
        return raw
          ...
          .replace(/==(.*?)==/g, '<mark>$1</mark>')       // 螢光筆
          .replace(/__(.*?)__/g, '<u>$1</u>')             // 底線 (雙底線)
          .replace(/_(.*?)_/g, '<u>$1</u>')               // 底線 (單底線)
          .replace(/\{(.*?)\}(.*?)\{\/\}/g, (match, color, content) => {
            const colorMap = {
              '紅': '#ff5f5f', 'red': '#ff5f5f',
              '藍': '#5fafff', 'blue': '#5fafff',
              '綠': '#78c96b', 'green': '#78c96b',
              '黃': '#ffd700', 'yellow': '#ffd700',
              '橘': '#ff9f43', 'orange': '#ff9f43',
              '灰': '#888888', 'gray': '#888888',
              '紫': '#a29bfe', 'purple': '#a29bfe',
              '黑': '#333333', 'black': '#333333'
            };
      ...
      // 沒有網址 → 直接顯示格式化後的文字
      if (!urlMatch) {
        return applyFormatting(text).replace(/\n/g, "<br>");
      }
      ...
      const formattedContent = applyFormatting(plainText).replace(/\n/g, "<br>");

      // 抓不到有效預覽 → 顯示原本包含網址的格式化文字
      ...
      // 有預覽卡片 → 顯示文字內容 + 卡片
      return `
        <div style="margin-bottom: ${plainText ? '8px' : '0'};">${formattedContent}</div>
        <div style="margin-top:6px;">
          <a href="${url}" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">
            <div style="border:1px solid #dcead3; background:#f4f8ef; border-radius:12px; overflow:hidden;">
              ${preview.image ? `
                <img src="${preview.image}" onerror="this.style.display='none';" 
                     style="width:100%; height:120px; object-fit:cover; display:block; background:#e9efe2;">
              ` : ""}
              <div style="padding:8px;">
                <div style="font-weight:700; color:#2f7438; font-size:13px;">${preview.title || ""}</div>
                ${preview.description ? `<div style="font-size:11px; color:#666; margin-top:4px;">${preview.description}</div>` : ""}
              </div>
            </div>
          </a>
        </div>
    ...
    if (sendMsgBtn) {
      sendMsgBtn.addEventListener("click", () => {
        ...
        const uid = (window.auth && window.auth.currentUser) ? window.auth.currentUser.uid : (window.getCurrentUserId ? window.getCurrentUserId() : "anonymous");
        guestbookColl.add({ name, text, time: Date.now(), uid })
    ...
    // --- 全域審計工具：V41.1 GPS 國家判定校驗 ---
    window.auditCountryByGps = async function({ dryRun = true, useApi = false } = {}) {
      if (!isAdmin) return console.warn("管理員權限不足，無法執行審計。");
      
      const cards = getPostcards();
      if (!cards || cards.length === 0) return console.log("目前沒有載入任何資料。");

      console.log(`[Audit] 開始執行國家判定審計 (總計: ${cards.length} 張, DryRun: ${dryRun})...`);
      const mismatches = [];
      let processed = 0;
      ...
      for (const card of cards) {
        ...
        let detected = "全球";
        if (useApi) {
          // 如果開啟使用 API，則針對每張卡片進行反查 (注意：Nominatim 有頻率限制)
          const info = await window.fetchReverseGeocode(card.lat, card.lng);
          detected = window.detectCountry(card.lat, card.lng, info?.address || card.address || card.locationText, info?.countryCode);
          if (detected === "全球" && info?.country) detected = info.country;
          // 延遲 1 秒以符合 Nominatim 使用政策
          await new Promise(r => setTimeout(r, 1000));
        } else {
          // 離線判定：使用改良後的系統性邏輯 (預設不主動發動 API 以免太慢)
          detected = await window.resolveCountrySystematic(card, useApi);
        }

        const saved = card.category || "全球";
        if (detected !== "全球" && detected !== saved) {
          mismatches.push({
            id: card.id,
            title: card.placeName || card.locationText || "無標題",
            lat: card.lat,
            lng: card.lng,
            savedCountry: saved,
            gpsCountry: detected,
            address: card.address || card.locationText,
            recommendFix: true
          });
        }
        processed++;
        if (processed % 20 === 0) console.log(`[Audit] 已處理 ${processed} 張...`);
      }

      console.table(mismatches);
      console.log(`[Audit] 審計完成。發現 ${mismatches.length} 筆不一致資料。`);

      if (!dryRun && mismatches.length > 0) {
        if (!confirm(`發現 ${mismatches.length} 筆不一致，確定要直接批次修正嗎？`)) return;
        let successCount = 0;
        for (const m of mismatches) {
          try {
            await updatePostcard(m.id, { category: m.gpsCountry });
            successCount++;
          } catch (e) { console.error(`更新 ${m.id} 失敗:`, e); }
        }
        alert(`修正完成！共更新 ${successCount} 筆資料。`);
      }
    ...
    window.handleFixGPSErrors = async () => {
      ...
      const confirmMsg = "這將根據最新的 GPS 規則掃描所有明信片並找出不一致的國家標籤。\n\n確定要開始執行審計嗎？";
      ...
      try {
        ...
        for (const card of cards) {
          if (card.lat && card.lng) {
            const detected = await window.resolveCountrySystematic(card, false);
            const saved = card.category || "全球";
            if (detected !== "全球" && detected !== saved) {
              mismatches.push({ id: card.id, gpsCountry: detected, savedCountry: saved });
            }
          }
          processed++;
          if (processed % 50 === 0 || processed === total) {
            btn.innerText = `⏳ 審查中 (${processed}/${total})...`;
          }
          // 每 100 筆稍微放慢一點點，避免阻礙 UI 渲染
          if (processed % 100 === 0) await new Promise(r => setTimeout(r, 0));
        }
        
        if (mismatches.length === 0) {
          alert("✨ 檢查完成！目前所有明信片的國家判定都正確。");
          return;
        }

        const fixConfirm = `發現 ${mismatches.length} 張明信片的國家標籤不符（例如：${mismatches[0].savedCountry} -> ${mismatches[0].gpsCountry}）。\n\n確定要一鍵修正所有標籤嗎？`;
        if (confirm(fixConfirm)) {
          btn.innerText = "⏳ 修正中...";
          let count = 0;
          for (const m of mismatches) {
            await updatePostcard(m.id, { category: m.gpsCountry });
            count++;
            btn.innerText = `⏳ 修正中 (${count}/${mismatches.length})...`;
          }
          alert(`🎉 修正完成！共更新了 ${count} 張明信片。`);
        }
      } catch (err) {
        console.error("Batch audit/fix failed:", err);
        alert("執行過程中發生錯誤。");
      } finally {
        btn.disabled = false;
  ...
  function showPreview(imageData, detectedTag) {
    ...
    // V41.1：確保每一種判定（含隱藏）都會顯示在畫面上告知使用者
    if (detectedTag) {
      if (tagInput) tagInput.value = detectedTag;
      setMessage(`✅ 圖片已載入，自動判定標籤：${detectedTag}`);
    } else {
      setMessage("✅ 圖片已載入");
    }
  }
  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      modalCardImage.src = getDisplayImageUrl(item);
      ...
      const canManage = isFirebaseAdmin || isOwner; // V39: 僅限管理員或已登入擁有者可編輯 (匿名上傳者無編輯權)
  ...
  function showToast(message) {
    ...
    const popular = [
    "全球", "台灣", "日本", "韓國", "香港", "澳門", "泰國", "新加坡", "馬來西亞", "越南", "菲律賓", "印度", "印尼", "蒙古", "杜拜", "沙烏地阿拉伯",
    "英國", "法國", "德國", "義大利", "西班牙", "葡萄牙", "瑞士", "荷蘭", "奧地利", "比利時", "盧森堡", "愛爾蘭", "丹麥", "挪威", "芬蘭", "瑞典", "冰島", 
    "捷克", "斯洛伐克", "匈牙利", "波蘭", "克羅埃西亞", "斯洛維尼亞", "羅馬尼亞", "保加利亞", "土耳其", "烏克蘭", "希臘", "美國", "加拿大", "墨西哥", 
    "巴西", "哥倫比亞", "阿根廷", "智利", "秘魯", "南非", "澳洲", "紐西蘭", "馬紹爾群島", "塞浦路斯", "密克羅尼西亞", "布拉格"
  ];
  ...
  function handleEditClick(item) {
    ...
    focusImage.src = getDisplayImageUrl(item) || "";
    ...
    async function submitEditChanges() {
      ...
      try {
        ...
        const updatedItem = await updatePostcardSmart(item.id, changes);
        if (updatedItem) {
          // V39: 立即強制刷新本地資料快取
          const idx = window.postcardsCache.findIndex(c => c.id === item.id);
          if (idx !== -1) {
            window.postcardsCache[idx] = updatedItem;
          }
          closeEditModal();
          safeRefreshViews(0); // 立即刷新
  ...
  initializeFirebaseStorage(function () {
    ...
    // V38: 節流刷新，避免連鎖反應導致的卡頓
    safeRefreshViews(isFirstLoad ? 0 : 300);

    // 這些功能需要等資料載入後執行，且資料變動時可能需要更新
    updateNoteSuggestions();
    updateFeaturedCard();
...
async function renderLocationDetails(item, container) {
  ...
  // 如果已經有詳細地址，進行背景一致性檢查
  if (item.placeName && item.address && item.category && item.category !== "全球") {
    const currentDetected = window.detectCountry(item.lat, item.lng, item.address, item.countryCode);
    if (currentDetected !== "全球" && currentDetected !== item.category) {
      console.log(`[Audit] 偵測到既有資料不一致，發動背景更新: ${item.category} -> ${currentDetected}`);
      autoUpdatePostcardDetails(item, { 
        placeName: item.placeName, 
        address: item.address, 
        countryCode: item.countryCode,
        country: currentDetected 
      });
    }
    return;
  }
  ...
  try {
    ...
    if (info) {
      ...
      const pName = document.getElementById("locPlaceName");
      const pAddr = document.getElementById("locAddress");
      if (pName) pName.textContent = "無法取得地點名稱";
      if (pAddr) pAddr.textContent = "無法取得地址資訊 (請確認網路或稍後再試)";

      // V41.1 保底：即便 API 失敗，也要根據最新座標規則嘗試修正國籍
      const detected = await window.resolveCountrySystematic(item, false);
      if (detected !== "全球" && detected !== item.category) {
        console.log(`[AutoFix] API 失敗但偵測到國籍誤判，執行保底修正: ${item.category} -> ${detected}`);
        autoUpdatePostcardDetails(item, { country: detected });
      }
    }
  } catch (e) {
...
// fetchReverseGeocode 已搬移至 locationParser.js
...
async function autoUpdatePostcardDetails(item, info) {
  if (typeof updatePostcardSmart === "function") {
    ...
    let newlyDetected = typeof window.detectCountry === "function" 
      ? window.detectCountry(item.lat, item.lng, info.address, info.countryCode) 
      : (info.country || "全球");
    ...
    // V41.1 強制同步邏輯：如果反查結果明確，且與舊分類不符，則強制更新
    if (newlyDetected && newlyDetected !== "全球" && item.category !== newlyDetected) {
      updates.category = newlyDetected;
      console.log(`[AutoFix] 偵測到國籍不一致，自動修正：${item.category} -> ${newlyDetected}`);
    }
