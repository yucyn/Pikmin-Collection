...
async function detectTagFromImage(imageDataUrl) {
  ...
  // 修正：擴大掃描高度至底部 (y: 180~400)
  const mushroomArea = ctx.getImageData(360, 180, 220, 220);
  ...
  for (let i = 0; i < mushroomArea.data.length; i += 4) {
    ...
    // 3. 偵測綠色 (蓋章) - 放寬藍色限制應對背景干擾
    if (g > 120 && g > r * 1.4 && g > b * 1.4 && r < 140 && b < 160) {
      clearedGreenCount++;
    }
  }

  // 蘑菇判定：極致門檻優化
  if (starYellowCount > 30 && textBlackCount > 40 && clearedGreenCount > 20) {
    return "蘑菇";
  }
