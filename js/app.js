...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function togglePreviewMode() {
    ...
    if (!user || user.isAnonymous) { alert("請先登入。🌱"); return; }
  ...
  function openCardModal(item) {
    ...
    const canManage = isAdmin || (currentUser && !currentUser.isAnonymous && typeof isOwnedByCurrentUser === "function" && isOwnedByCurrentUser(item));
  ...
  function handleEditClick(item) {
    ...
    saveBtn.onclick = async (e) => {
      ...
      const loc = parseLocation(locIn.value); if (!loc) return alert("座標格式錯誤");
  ...
  function ensureEditModal() {
    ...
    modal.innerHTML = `
      <div class="edit-modal-backdrop" onclick="this.parentNode.classList.add('hidden'); document.body.classList.remove('modal-open')"></div>
      <form id="editForm" class="edit-modal-panel">
        ...
        <div class="sidebar-form-grid">
          ...
          <div class="field-half"><label>國家</label><select id="editCategoryInput"><option value="全球">全球</option><option value="台灣">台灣</option><option value="日本">日本</option></select></div>
          <div class="field-half"><label>標籤</label><select id="editTagInput"><option value="花">花</option><option value="蘑菇">蘑菇</option><option value="隱藏">隱藏</option></select></div>
        </div>
  ...
  function updateFeaturedCard() {
    const cards = typeof getPostcards === "function" ? getPostcards() : []; if (!cards.length) return;
    ...
    if (container && item) {
      ...
      if (title) title.textContent = item.locationText;
  ...
  initializeFirebaseStorage(() => { initAppFeatures(); safeRefreshViews(); });
});
...
========================================================= */
(function() {
  const selectors = [".map-mode .map-list", ".map-mode .map-results"];
  function bind(el) {
