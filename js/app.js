...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleEditClick(item) {
    ...
    if (saveBtn) {
      ...
      newSaveBtn.style.zIndex = "10000"; // 確保層級最高
      ...
      const handleSaveTrigger = async (e) => {
        ...
        console.log("Save trigger fired");
        
        // 強制停止所有拖拽狀態
        if (typeof stopDrag === "function") stopDrag();
        if (typeof isDragging !== "undefined") isDragging = false; 
        
        if (newSaveBtn.disabled) return;
        
        newSaveBtn.disabled = true;
        newSaveBtn.textContent = "儲存中...";
        ...
        try {
          ...
          newSaveBtn.textContent = "儲存變更";
        }
      };

      newSaveBtn.addEventListener("click", handleSaveTrigger);
      // 增加 touchstart 提升手機反應，但需防止與 click 雙重觸發
      newSaveBtn.addEventListener("touchstart", (e) => {
        if (!newSaveBtn.disabled) {
           // 不阻止預設行為，讓 click 正常觸發，但先給予視覺回饋
...
async function autoUpdatePostcardDetails(item, info) {
  if (typeof updatePostcardSmart === "function") {
    ...
    // 如果目前分類是「全球」或空白，且抓到了明確的國家名稱，就自動更新分類
    if ((!item.category || item.category === "全球") && info.country) {
      updates.category = info.country;
      console.log(`[AutoFix] 將明信片 ${item.id} 的分類從 全球 更新為 ${info.country}`);
    }
