document.addEventListener("DOMContentLoaded", function () {
  ...
  // --- 最高權限：公告、統計、留言板邏輯 ---
  try {
    const bulletinRef = db.ref("settings/bulletin");
    const statsRef = db.ref("stats/visitors");
    ...
    bulletinRef.on("value", (snapshot) => {
      const text = snapshot.val() || "歡迎來到皮克敏收藏站！";
    ...
    if (noticeTitle) {
      noticeTitle.addEventListener("click", () => {
        ...
        if (titleClickCount >= 5) {
          ...
          alert("管理員模式已開啟 🛡️ (已啟用留言刪除權限)");
          refreshGuestbook(); // 重新渲染留言板以顯示刪除按鈕
          titleClickCount = 0;
        }
      });
    }
    ...
    statsRef.on("value", (snapshot) => {
      if (viewCountEl) viewCountEl.innerText = snapshot.val() || 0;
    });
    ...
    const refreshGuestbook = () => {
      guestbookRef.limitToLast(20).on("value", (snapshot) => {
        const msgs = [];
    ...
    // 暴露刪除函數給全局 (因為是 HTML inline onclick)
    ...
    if (saveBulletinBtn) {
      saveBulletinBtn.addEventListener("click", async () => {
        try {
          await bulletinRef.set(bulletinInput.value);
          alert("公告已更新！✨");
