...
function renderPostcards({
  ...
  list.forEach(item => {
    ...
    const hasBeenThere = currentUser && beenThereBy.includes(currentUser.uid);
    const beenThereBtn = createEl("button", `float-btn been-there-btn ${hasBeenThere ? "active" : ""}`, `👤 ${formatLikeCount(item.beenThereCount || 0)}`);
    beenThereBtn.title = "我也去過";
    beenThereBtn.onclick = (e) => { e.stopPropagation(); if(window.togglePostcardBeenThere) window.togglePostcardBeenThere(item.id); };
    actionsGroup.appendChild(beenThereBtn);
