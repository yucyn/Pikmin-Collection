...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initAppFeatures() {
    ...
    function enableAdminUI() {
      ...
      const adminTools = document.getElementById("adminToolsArea");
  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      modalCardLocation.textContent = `${item.category || "全球"}｜${locationText}｜${isOwnedByCurrentUser(item) ? "我的明信片" : "公開明信片"}｜${isLikedByCurrentUser(item) ? "❤️" : "🤍"} ${formatLikeCount(item.likeCount)}${noteSuffix}`;
      ...
      const isAdmin = window.PikminAuthGate && typeof window.PikminAuthGate.isFirebaseAdmin === "function" && window.PikminAuthGate.isFirebaseAdmin();
      const isOwner = typeof isOwnedByCurrentUser === "function" && isOwnedByCurrentUser(item);
      const canManage = isAdmin || (isRealUser && isOwner);
