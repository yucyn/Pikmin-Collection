...
window.initializeFirebaseStorage = function(onChange) {
  ...
  // V38+: 增強監聽穩定性。針對手機版切換分頁或網路不穩導致 snapshot 斷線的問題，建立自動重連機制。
  let unsubscribe = null;
  const startSnapshot = () => {
    if (unsubscribe) {
      console.log("🔄 重新同步明信片資料監聽器...");
      unsubscribe();
    }
    unsubscribe = window.db.collection(collectionName).onSnapshot(snapshot => {
      ...
      // 讀取失敗時不清除舊快取，確保使用者仍能看到離線資料，並安排重試
      setTimeout(startSnapshot, 5000);
      onChange(); 
    });
  };
  startSnapshot();

  // 當使用者回到分頁時 (手機喚醒或切換 APP 回來)，強制檢查連線並重刷資料，避免畫面卡死
