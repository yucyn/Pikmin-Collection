...
async function compressImageDataUrl(originalDataUrl) {
  ...
  while (getDataUrlSizeBytes(compressedDataUrl) > FIRESTORE_SAFE_LIMIT_BYTES && quality > 0.35) {
    quality -= 0.1;
...
function readImageFile(file, onLoaded, onError) {
  ...
  reader.onload = async function (event) {
    try {
      ...
      const size = getDataUrlSizeBytes(compressedDataUrl);

      if (size > FIRESTORE_SAFE_LIMIT_BYTES) {
  ...
  try {
    ...
    return await new Promise((resolve, reject) => {
      ...
      reader.onload = async event => {
        try {
          resolve(await compressImageDataUrl(event.target.result));
        } catch (error) {
          reject(error);
        }
      };
