...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initAppFeatures() {
    ...
    function linkifyText(text) {
      ...
      return safe
        .replace(/(https?:\/\/[^\s<]+)/g, function (url) {
          ...
          return `<a href="${cleanUrl}" target="_blank" rel="noopener noreferrer" style="color:#4CAF50;text-decoration:underline;">${cleanUrl}</a>${tail}`;
        })
        .replace(/\n/g, "<br>");
    }

    function normalizeBulletinText(text) {
      return String(text || "")
        .replace(/\r\n/g, "\n")
        .replace(/[ \t]+\n/g, "\n")
        .replace(/\n{4,}/g, "\n\n\n")
    ...
    function classifyBulletin(text) {
      const lower = String(text || "").toLowerCase();
      if (/event|campaign|community day|challenge|mushroom|活動|挑戰|社群日|蘑菇|花/.test(lower)) return "活動";
      if (/guide|tips|how to|攻略|教學|技巧|指南/.test(lower)) return "攻略";
      if (/update|news|version|maintenance|announcement|更新|維護|公告|新消息|版本/.test(lower)) return "更新";
      return "情報";
    }
    ...
    async function fetchLinkPreview(url) {
      ...
      try {
        ...
        console.warn("公告網址預覽讀取失敗:", err);
        return null;
      }
    }

    async function buildForwardBulletinText(rawText) {
      const original = normalizeBulletinText(rawText);
      const url = extractFirstUrl(original);
      if (!url) return original;

      const preview = await fetchLinkPreview(url);
      if (!preview || (!preview.title && !preview.description)) return original;

      const title = normalizeBulletinText(preview.title || "");
      const description = normalizeBulletinText(preview.description || "");
      const category = classifyBulletin(`${title}\n${description}\n${original}`);

      // 如果使用者已經自己整理過很多內容，就只保留原文，不強制覆蓋
      const originalWithoutUrl = normalizeBulletinText(original.replace(url, ""));
      if (originalWithoutUrl.length > 40) return original;

      return [
        `【${category}】${title || "Pikmin 最新情報"}`,
        description,
        url
      ].filter(Boolean).join("\n");
    }
    ...
    const startCarousel = () => {
      ...
      bulletinInterval = setInterval(() => {
        ...
        setTimeout(() => {
          ...
          bulletinContent.innerHTML = linkifyText(bulletinLines[currentLineIndex]);
    ...
    settingsDoc.onSnapshot((doc) => {
      if (doc.exists) {
        ...
        bulletinContent.innerHTML = linkifyText(bulletinLines[0] || "歡迎！");
        bulletinInput.value = text;
        startCarousel();
      } else {
        bulletinContent.innerHTML = linkifyText("歡迎來到皮克敏收藏站！");
      }
    ...
    if (saveBulletinBtn) {
      saveBulletinBtn.addEventListener("click", async () => {
        if (!isAdmin) return;
        try {
          const finalBulletinText = await buildForwardBulletinText(bulletinInput.value);
          bulletinInput.value = finalBulletinText;
          await settingsDoc.set({ text: finalBulletinText }, { merge: true });
          alert("公告已更新！✨");
    ...
    window.saveTitleUpdate = async (id, val) => {
      if (!isAdmin) return;
      try {
        if (id === "bulletinContent") await settingsDoc.set({ text: normalizeBulletinText(val) }, { merge: true });
