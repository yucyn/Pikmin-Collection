(function () {
  ...
  function shouldLockItem(item) {
    ...
    if (isUnlocked()) return false; // 管理員或已解鎖者不鎖定
    
    // 擁有者（作者）不應被鎖定自己的卡片
    if (typeof window.isOwnedByCurrentUser === "function" && window.isOwnedByCurrentUser(item)) return false;
    
    // 新 V38 邏輯：優先判斷審核與可見性設定
    if (item.reviewStatus === "approved") {
      if (item.visibility === "public") return false;
      if (item.visibility === "members") return true; // 未解鎖者看到會員卡片為鎖定
    }
    
    // 優先判斷手動公開設定
    if (item.publicUnlock === true) return false;
    if (item.publicUnlock === false) return true;

    // 舊資料/相容邏輯 (處理沒有新欄位的舊明信片)
    if (item.requirePlayerUnlock === true) return true;
    if (isMushroomItem(item)) return true; // 舊的蘑菇標籤預設鎖定
  ...
  function injectForcedStyles() {
    ...
    style.textContent = `
        ...
        /* 調整按鈕透明度與樣式 - V38 優化：移除昂貴的 backdrop-filter */
        .v37-mushroom-locked .postcard-coords {
        ...
        .modal-heart-btn {
            ...
            border-radius: 8px !important; /* 縮小圓角 */
        }
        /* 強制縮小飛機分享按鈕 */
    ...
    wrap.innerHTML = `
      <section class="v37-auth-panel" role="dialog" aria-modal="true" aria-labelledby="v37AuthTitle">
        <div class="v37-auth-header">
          <div>
            <h2 class="v37-auth-title" id="v37AuthTitle">待解鎖</h2>
