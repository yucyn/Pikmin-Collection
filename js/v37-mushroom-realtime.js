(function() {
                ...
                <div class="mr-modal-panel">
                    ...
                    <div class="mr-modal-content">
                        ...
                        <div class="mr-form-group">
                            ...
                            <select id="mrInputTicket" class="mr-form-select"><option value="免券">免券</option></select>
                        </div>
                        <div class="mr-form-group">
                            <label class="mr-form-label">最低戰力</label>
                            <input id="mrInputPower" type="text" class="mr-form-input" placeholder="例如: 4000 或 都可">
                        </div>
                        <div class="mr-form-group">
                        ...
                        <button id="mrSubmitPublish" class="mr-btn mr-btn-publish" style="width:100%; justify-content:center; padding:12px; margin-top:10px;">發布情報</button>
                    </div>
                </div>
            </div>
            ...
            <div id="mrModalApply" class="mr-modal-backdrop">
                <div class="mr-modal-panel">
                    ...
                    <div class="mr-modal-content">
                        ...
                        <button id="mrSubmitApply" class="mr-btn mr-btn-publish" style="width:100%; justify-content:center; padding:12px;">送出申請</button>
                    </div>
                </div>
            </div>
        ...
        scroller.innerHTML = mushroomTasks.map(t => {
            ...
            return `
                <div class="mr-task-card ${isFull ? 'full' : ''}" onclick="handleTaskClick('${t.id}')">
                    ...
                    <span class="mr-task-type">${t.size}${t.type}</span> / 
    ...
    window.handleTaskClick = function(id) {
        ...
        if (t.mode === 'fly') {
            ...
            info.innerHTML = `✨ ${t.size}${t.type} / 缺${t.needed} / 限${t.power}`;
    ...
    async function submitTask() {
        const user = firebase.auth().currentUser;
        if (!user) { alert("請先登入才能發布情報唷！"); return; }
        ...
        const data = {
            ...
            mode: document.querySelector('.mr-mode-tab.active').dataset.mode,
            coords: document.getElementById('mrInputCoords').value,
            ownerId: user.uid,
            status: 'active',
            createdAt: Date.now()
        };
            ...
            return `
                <div style="border:1px solid #e1f0de; border-radius:16px; padding:15px; margin-bottom:15px; background:#fff;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                        <strong style="color:#4a6a43;">✨ ${t.size}${t.type} / 缺${t.needed} / ${t.mode === 'fly' ? '📍自飛' : '📨邀請'}</strong>
    ...
    window.handleApprove = async function(appId, taskId) {
        try {
            ...
            await db.runTransaction(async trans => {
                ...
                const newNeeded = Math.max(0, snap.data().needed - 1);
                trans.update(tRef, { 
                    needed: newNeeded,
                    status: newNeeded === 0 ? 'full' : 'active'
                });
            });
            renderMyTasks();
    ...
    function openModal(type) {
        closeAllModals();
        if (type === 'publish') document.getElementById('mrModalPublish').classList.add('show');
