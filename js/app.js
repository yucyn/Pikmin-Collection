document.addEventListener("DOMContentLoaded", function () {
  ...
  function setView(view) {
    ...
    if (isCollage && window.CollageModule) {
      window.CollageModule.refreshLibrary(getFilteredPostcards(getFilters()));
    }
  }
  ...
  if (window.CollageModule) {
    window.CollageModule.init();
  }


  // ===== Collage Module Button Safety Patch =====
  // 這段確保即使原本 setView 沒有成功整合，拼貼模式按鈕也一定可以切換。
  const collageButtonPatch = document.getElementById("collageViewBtn");
  const collageViewPatch = document.getElementById("collageView");

  if (collageButtonPatch && collageViewPatch) {
    collageButtonPatch.addEventListener("click", function () {
      collectionView.classList.add("hidden");
      mapView.classList.add("hidden");
      collageViewPatch.classList.remove("hidden");

      collectionViewBtn.classList.remove("active");
      mapViewBtn.classList.remove("active");
      collageButtonPatch.classList.add("active");
    ...
    collectionViewBtn.addEventListener("click", function () {
      collageViewPatch.classList.add("hidden");
      collageButtonPatch.classList.remove("active");
    });
