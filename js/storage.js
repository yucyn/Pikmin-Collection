...
window.initializeFirebaseStorage = function(onChange) {
  ...
  const startSnapshot = () => {
    ...
    unsubscribe = window.db.collection(collectionName).onSnapshot(snapshot => {
      ...
      // 快速比對：長度、前 5 項 ID 的組合、以及總喜歡數的加總
      const newTotalLikes = newData.reduce((sum, item) => sum + (item.likeCount || 0), 0);
      const oldTotalLikes = window.postcardsCache.reduce((sum, item) => sum + (item.likeCount || 0), 0);
      
      const newIds = newData.slice(0, 5).map(i => i.id).join("|");
      const oldIds = window.postcardsCache.slice(0, 5).map(i => i.id).join("|");

      const isSame = window.postcardsCache.length === newData.length && 
                     newTotalLikes === oldTotalLikes &&
                     newIds === oldIds;

      if (!isSame || window.postcardsCache.length === 0) {
