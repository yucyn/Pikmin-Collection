...
function normalizePostcard(id, data) {
  ...
  return {
    ...
    /** 管理員：非蘑菇卡片也可標記為須完成玩家登入才顯示圖片／GPS */
    requirePlayerUnlock: Boolean(data.requirePlayerUnlock),
    /** 管理員：蘑菇預設鎖定時，勾選後改為公開（訪客可直接看） */
    publicUnlock: Boolean(data.publicUnlock),
...
function initializeFirebaseStorage(onChange) {
  ...
  // 不可在 Google 從持久化還原「之前」就 signInAnonymously()，否則匿名會搶先／覆蓋，玩家會一直變回匿名 → 登入循環。
  let authBootstrapDone = false;
  let deferredAnonTimer = null;
  const DEFER_ANON_MS = 2000;

  function clearDeferredAnonymous() {
    if (deferredAnonTimer) {
      clearTimeout(deferredAnonTimer);
      deferredAnonTimer = null;
    }
  }
  ...
  function scheduleDeferredAnonymousSignIn() {
    clearDeferredAnonymous();
    deferredAnonTimer = setTimeout(() => {
      deferredAnonTimer = null;
      if (!auth.currentUser) {
        tryAnonymousSignIn();
      }
  ...
  auth.onAuthStateChanged(user => {
    ...
    if (authBootstrapDone) {
      scheduleDeferredAnonymousSignIn();
    }
  });
