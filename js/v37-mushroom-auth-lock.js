(function () {
  ...
  function getValidGameInviteCodes() {
  ...
  function updateButtonState() {
    ...
    btn.title = unlocked ? "已登入／已解鎖圖片與 GPS" : "解鎖蘑菇 · 登入";
    btn.textContent = "🌱";
  }
  ...
  function saveNicknameAndInvite() {
    ...
    const inviteDigits = normalizeInviteDigits(inviteRaw);
    if (!inviteDigits) {
      setStatus("🌱 請填寫遊戲邀請碼。");
      return false;
    }
    if (inviteDigits.length !== 12) {
      setStatus("🌱 邀請碼需為 12 碼");
      return false;
    }

    const validList = getValidGameInviteCodes();
    if (!validList.length) {
      setStatus("🌱 尚未設定有效邀請碼，請聯絡管理員。");
      return false;
    }

    if (!validList.includes(inviteDigits)) {
