...
function renderPostcards({
  ...
  list.forEach(item => {
    ...
    const isPriorityImage = (currentPage === 1 && list.indexOf(item) < 4);
    if (isPriorityImage) {
      image.loading = "eager";
    ...
    if (isAdmin) {
      const badgeMap = {
        ...
        approved: isHidden
          ? { cls: "review-badge review-badge--hidden",   text: "🚫 隱藏" }
          : { cls: "review-badge review-badge--approved", text: "✅ 已通過" },
        rejected: { cls: "review-badge review-badge--rejected", text: "❌ 未通過" }
      };
      const badgeInfo = isHidden ? badgeMap.approved :
                        (badgeMap[reviewStatus] || badgeMap.approved);
      ...
      if (reviewStatus === "pending") {
        const approveBtn = createEl("button", "review-btn review-btn--approve", "✅通過");
        approveBtn.title = "審核通過，公開顯示";
        approveBtn.onclick = async (e) => {
          e.stopPropagation();
          approveBtn.disabled = true;
          try {
            await reviewPostcard(item.id, "approve");
            card.remove();
          } catch (err) { alert("審核失敗: " + err.message); approveBtn.disabled = false; }
        };
        const rejectBtn = createEl("button", "review-btn review-btn--reject", "返回7天");
        rejectBtn.title = "退回，7天後刪除";
        rejectBtn.onclick = async (e) => {
          e.stopPropagation();
          rejectBtn.disabled = true;
          try {
            await reviewPostcard(item.id, "reject");
            card.remove();
          } catch (err) { alert("操作失敗: " + err.message); rejectBtn.disabled = false; }
        };
        const deleteBtn = createEl("button", "review-btn review-btn--delete", "🗑刪除");
        deleteBtn.title = "直接刪除此卡片";
        deleteBtn.onclick = async (e) => {
          e.stopPropagation();
          if (!confirm("確定直接刪除此明信片？")) return;
          deleteBtn.disabled = true;
          try {
            await deletePostcard(item.id);
            card.remove();
          } catch (err) { alert("刪除失敗: " + err.message); deleteBtn.disabled = false; }
        };
        reviewBar.appendChild(approveBtn);
        reviewBar.appendChild(rejectBtn);
        reviewBar.appendChild(deleteBtn);
      } else {
        // 非 pending 狀態：提供公開/隱藏切換
        const toggleBtn = isHidden || reviewStatus === "rejected"
          ? createEl("button", "review-btn review-btn--approve", "👁️公開")
          : createEl("button", "review-btn review-btn--hide",    "🚫隱藏");
        toggleBtn.onclick = async (e) => {
          e.stopPropagation();
          toggleBtn.disabled = true;
          try {
            const nextAction = (isHidden || reviewStatus === "rejected") ? "show" : "hide";
            await reviewPostcard(item.id, nextAction);
            card.remove();
          } catch (err) { alert("操作失敗: " + err.message); toggleBtn.disabled = false; }
        };
        const deleteBtn2 = createEl("button", "review-btn review-btn--delete", "🗑刪除");
        deleteBtn2.onclick = async (e) => {
          e.stopPropagation();
          if (!confirm("確定刪除此明信片？")) return;
          deleteBtn2.disabled = true;
          try { await deletePostcard(item.id); card.remove(); }
          catch (err) { alert("刪除失敗: " + err.message); deleteBtn2.disabled = false; }
        };
        reviewBar.appendChild(toggleBtn);
        reviewBar.appendChild(deleteBtn2);
      }
      ...
      // 登入會員看到自己的待審 / 未通過標籤
      if (reviewStatus === "pending") {
        const badge = createEl("div", "review-badge review-badge--pending", "⏳ 待審核");
        photo.appendChild(badge);
      } else if (reviewStatus === "rejected") {
        const badge = createEl("div", "review-badge review-badge--rejected");
        const deleteAfterDate = item.deleteAfter
          ? new Date(item.deleteAfter).toLocaleDateString("zh-TW")
          : "未知日期";
        badge.textContent = `❌ 未通過，將於 ${deleteAfterDate} 刪除`;
        photo.appendChild(badge);
      }
    }
