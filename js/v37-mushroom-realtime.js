(function() {
    ...
    const CLEANUP_MS = 30 * 60 * 1000; // 縮短為 30 分鐘過期，減少讀取量
    ...
    async function checkAndAutoDeleteTasks() {
        ...
        // 2. 處理「發布超過 3 小時」 (去資料庫抓取，處理那些被 30 分鐘過濾器擋掉的老舊資料)
        try {
            const oldTasksSnap = await db.collection(TASKS_COLL)
                .where('ownerId', '==', user.uid)
                .where('createdAt', '<', now - THREE_HOURS)
                .get();
            
            for (const doc of oldTasksSnap.docs) {
                console.log(`[AutoCleanup] 任務發布超過 3 小時，自動刪除: ${doc.id}`);
    ...
    function upgradeBar(el) {
        ...
        el.innerHTML = `
            ...
            <div style="font-size: 10px; color: #9ca3af; margin-top: 2px; text-align: right; opacity: 0.8;">✨: 未標示 / ※ 額滿 3 分鐘或發布 3 小時後自動刪除</div>
        `;
                ...
                <div class="mr-modal-panel">
                    ...
                    <div class="mr-modal-content">
                        ...
                        <div class="mr-form-group">
                            ...
                            <input id="mrInputOwnerNick" type="text" class="mr-form-input" placeholder="">
                        </div>
                        <div class="mr-form-group">
    ...
    function listenToTasks() {
        ...
        unsubscribeTasks = db.collection(TASKS_COLL)
          ...
          .limit(20) // 從 50 降到 20，進一步減少單次讀取量
          .onSnapshot(snap => {
            mushroomTasks = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            renderTaskBar();
            // 不在這裡主動呼叫 renderMyTasks，交給 listenToMyApplications 觸發
        });
    }
            ...
            return `
                <div style="border:1px solid #e1f0de; border-radius:16px; padding:15px; margin-bottom:15px; background:#fff;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                        <strong style="color:#4a6a43;">${starsDisplay} ${displayType} / 缺${t.needed} / ${t.mode === 'fly' ? '📍自飛' : '📨邀請'}</strong>
