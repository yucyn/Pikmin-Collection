    ...
    const doUpdate = () => {
      ...
      if (modalLikeBtn) {
        ...
        modalLikeBtn.onclick = async (e) => {
          ...
          if (modalLikeBtn.dataset.likeBusy === "true") return;
          modalLikeBtn.dataset.likeBusy = "true";
          ...
          try {
            ...
            console.error("愛心寫入失敗：", error);

            // 失敗就把按鈕還原
            modalLikeBtn.innerHTML = `${wasLiked ? "❤️" : "🤍"} <span class="modal-like-count">${formatLikeCount(currentCount)}</span>`;
            modalLikeBtn.classList.toggle("active", wasLiked);

            alert("愛心操作失敗，請稍後再試");
          } finally {
            modalLikeBtn.dataset.likeBusy = "false";
          }
        };
      }
