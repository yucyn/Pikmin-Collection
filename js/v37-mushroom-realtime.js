(function() {
    ...
    const REPORTS_COLL = "reports"; // 統一回報集合名稱
        ...
        el.innerHTML = `
            <div class="mr-bar-header">
                ...
                <div class="mr-bar-actions">
                    ...
                    <button class="mr-btn mr-btn-report" id="mrReportBtn">回報資料</button>
                </div>
            </div>
    ...
    function listenToMyApplications() {
        ...
        firebase.auth().onAuthStateChanged(user => {
            // 更新回報按鈕文字 (訪客或未登入顯示「登入回報」)
            const reportBtn = document.getElementById('mrReportBtn');
            if (reportBtn) {
                reportBtn.textContent = (user && !user.isAnonymous) ? "回報資料" : "登入回報";
            }
    ...
    async function submitReport() {
        ...
        try {
            // 使用專屬的 reports 集合，並確保使用當前的 db 實例
            const firestore = firebase.firestore();
            await firestore.collection(REPORTS_COLL).add({
                type, 
                desc,
                uid: user.uid,
                userEmail: user.email || "N/A",
                createdAt: Date.now(),
                source: "mushroom_system"
            });
            ...
            alert("通報失敗：" + e.message + "\n請確認 Firebase Rules 已正確設定 match /reports/{id}");
        }
    }
