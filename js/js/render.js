...
// 分塊渲染：大量 base64 圖片一次插入會卡死主執行緒（GitHub Pages / Firestore 全量同步時特別明顯）
var __pikminUiRenderGen = 0;
function beginPikminUiRenderPass() {
  return ++__pikminUiRenderGen;
}
var PIKMIN_GRID_CHUNK = 20;
var PIKMIN_MAP_CHUNK = 24;

function isPikminUiRenderStale(renderPassGen) {
  return Boolean(renderPassGen) && renderPassGen !== __pikminUiRenderGen;
}

function buildPostcardCard(item, {
  onCardClick,
  onLikeClick,
  onCopyClick,
  onDeleteClick,
  ...
  const focusX = clampPercent(item.imageFocusX);
  const focusY = clampPercent(item.imageFocusY);
...
function buildMapListButton(item, onSelect) {
...
function renderPostcards({
  ...
  let index = 0;
  const firstSync = Math.min(PIKMIN_GRID_CHUNK, list.length);
  const headFragment = document.createDocumentFragment();
  for (; index < firstSync; index++) {
    headFragment.appendChild(buildPostcardCard(list[index], handlers));
  }
  grid.appendChild(headFragment);

  function appendChunk() {
    if (isPikminUiRenderStale(renderPassGen)) return;

    const fragment = document.createDocumentFragment();
    const limit = Math.min(index + PIKMIN_GRID_CHUNK, list.length);

    for (; index < limit; index++) {
      fragment.appendChild(buildPostcardCard(list[index], handlers));
    }
...
function renderMapList({ mapList, onSelect, filters, renderPassGen = 0 }) {
  const list = getFilteredPostcards(filters);
  mapList.innerHTML = "";
  ...
  let index = 0;
  const firstSync = Math.min(PIKMIN_MAP_CHUNK, list.length);
  const headFragment = document.createDocumentFragment();
  for (; index < firstSync; index++) {
    headFragment.appendChild(buildMapListButton(list[index], onSelect));
  }
  mapList.appendChild(headFragment);

  function appendChunk() {
    if (isPikminUiRenderStale(renderPassGen)) return;

    const fragment = document.createDocumentFragment();
    const limit = Math.min(index + PIKMIN_MAP_CHUNK, list.length);

    for (; index < limit; index++) {
      fragment.appendChild(buildMapListButton(list[index], onSelect));
    }
