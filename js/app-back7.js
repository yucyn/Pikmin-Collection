  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      if (modalLikeBtn) {
        ...
        modalLikeBtn.innerHTML = `${isLiked ? "❤️" : "🤍"} <span class="modal-like-count">${formatLikeCount(item.likeCount)}</span>`;
        modalLikeBtn.classList.toggle("active", isLiked);
        modalLikeBtn.onclick = async (e) => {
          e.stopPropagation();
          // 改回直接調用 togglePostcardLike，因為 onLikeClick 不是全域函式
          if (typeof togglePostcardLike === "function") {
            // 樂觀更新：立即切換狀態供視覺反饋
            const wasLiked = modalLikeBtn.classList.contains("active");
            const nextLiked = !wasLiked;
            const currentCount = parseInt(modalLikeBtn.querySelector(".modal-like-count")?.textContent || "0");
            ...
            // 同步刷新外部視圖
            if (typeof safeRefreshViews === "function") safeRefreshViews();
            
            // 稍後再次確認最終狀態（從快取抓取最新資料）
            const updatedItem = getPostcardById(item.id);
            if (updatedItem) {
              const finalLiked = isLikedByCurrentUser(updatedItem);
              modalLikeBtn.innerHTML = `${finalLiked ? "❤️" : "🤍"} <span class="modal-like-count">${formatLikeCount(updatedItem.likeCount)}</span>`;
