(function () {
  ...
  /**
   * 主內容在 .main { overflow:auto } 內捲動時，單純 fixed 在 Chrome/Safari 仍可能與版面不同步。
   * 用「fixed 容器 + Visual Viewport 幾何」貼齊實際可見區域（含行動版網址列／縮放）。
   */
  let v37VvPinBound = false;
  let v37VvRaf = null;

  function syncV37OverlayGeometry() {
    ...
    const apply = (el) => {
      ...
      if (vv) {
        ...
        el.style.setProperty("left", `${vv.offsetLeft}px`, "important");
        el.style.setProperty("width", `${vv.width}px`, "important");
    ...
    apply(document.getElementById("v37FixedUiRoot"));
    apply(document.getElementById("v37AuthPanelBackdrop"));
  }

  function scheduleSyncV37OverlayGeometry() {
    if (v37VvRaf) return;
    v37VvRaf = requestAnimationFrame(() => {
      v37VvRaf = null;
      syncV37OverlayGeometry();
    });
  }

  function bindV37VisualViewportPinning() {
    if (v37VvPinBound) return;
    v37VvPinBound = true;
    const vv = window.visualViewport;
    if (vv) {
      vv.addEventListener("scroll", scheduleSyncV37OverlayGeometry, { passive: true });
      vv.addEventListener("resize", scheduleSyncV37OverlayGeometry);
    }
    window.addEventListener("resize", scheduleSyncV37OverlayGeometry);
    window.addEventListener("scroll", scheduleSyncV37OverlayGeometry, { passive: true, capture: true });
    window.addEventListener("pageshow", (ev) => {
      if (ev.persisted) scheduleSyncV37OverlayGeometry();
    });
  }
  ...
  function ensureFixedUiRoot() {
    ...
    scheduleSyncV37OverlayGeometry();
    return root;
  }
