...
const POSTCARDS_CACHE_KEY = "pikmin_postcards_data_cache";

// 從 LocalStorage 載入快取，讓初始啟動達到近乎 0 秒的體驗
let postcardsCache = [];
try {
  const localData = localStorage.getItem(POSTCARDS_CACHE_KEY);
  if (localData) {
    postcardsCache = JSON.parse(localData);
  }
} catch (e) {
  console.warn("無法讀取明信片快取", e);
}
...
function initializeFirebaseStorage(onChange) {
  ...
  db.collection(collectionName).onSnapshot(snapshot => {
    ...
    // 更新快取
    try {
      localStorage.setItem(POSTCARDS_CACHE_KEY, JSON.stringify(postcardsCache));
    } catch (e) {
      console.warn("快取寫入失敗 (可能是資料量過大)", e);
    }
