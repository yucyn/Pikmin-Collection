...
document.addEventListener("DOMContentLoaded", function () {
  ...
  `;
    ...
    const updatePresence = () => {
      ...
      presenceColl.doc(uid).set({ lastSeen: Date.now() }, { merge: true });
    };
    ...
    // V38：在線人數改成低頻讀取，不使用 onSnapshot，避免每次 reload 建立高成本即時監聽
    async function refreshOnlineCount() {
      if (!onlineCountEl) return;
      try {
        const snapshot = await presenceColl.get();
        const now = Date.now();
        const fiveMinsAgo = now - 300000;
        let onlineCount = 0;
        snapshot.forEach(doc => {
          if (doc.data().lastSeen > fiveMinsAgo) onlineCount++;
        });
        onlineCountEl.innerText = onlineCount;
      } catch (err) {
        console.warn("在線人數讀取失敗:", err);
      }
    }
    refreshOnlineCount();
  ...
  function openCardModal(item) {
    ...
    const doUpdate = () => {
      ...
      modalMapLink.textContent = isV37Locked ? "🍄解鎖開啟地圖" : "Open Map";
  ...
  function bindTagFilterButtons() {
    ...
    // 把 mobileTagMenu 移進 #v37LoginBtnHost（position:fixed + translateZ(0) GPU層）
    // 用 position:absolute 在裡面定位，top/right = viewport 座標，不受頁面捲動影響
    const tagMenuHost = document.getElementById("v37LoginBtnHost") || document.body;
    if (mobileTagMenu && mobileTagMenu.parentNode !== tagMenuHost) {
      tagMenuHost.appendChild(mobileTagMenu);
    }

    const placeMobileTagMenuNearButton = () => {
      if (!mobileTagMenu || !mobileTagFilterBtn) return;
      ...
      mobileTagMenu.style.right = "8px";
      mobileTagMenu.style.left = "auto";
      mobileTagMenu.style.bottom = "auto";
      mobileTagMenu.style.minWidth = "140px";
    };
    ...
    if (mobileTagFilterBtn) {
      ...
      mobileTagFilterBtn.onclick = function(e) {
        ...
        if (mobileTagMenu) {
          const willOpen = mobileTagMenu.classList.contains("hidden");
          if (willOpen) {
  ...
  function updateSidebarCardCount() {
    ...
    try {
      ...
      el.textContent = String(getPostcards().length);
    } catch (e) {
      el.textContent = "—";
    }
  }
