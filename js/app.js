document.addEventListener("DOMContentLoaded", function () {
  ...
  // --- 最高權限：公告、統計、留言板邏輯 (Firestore 版本) ---
  let isAdminInitialized = false;
  function initAdminLogic() {
    if (isAdminInitialized || !db) return;
    ...
    try {
      ...
      const noticeTitle = document.querySelector(".sidebar-notice .notice-title span");
      ...
      function enableAdminFeatures() {
        ...
        if (visitorStats) visitorStats.style.display = "inline-block";
        ...
        document.querySelectorAll("#bulletinContent").forEach(el => {
          ...
          el.addEventListener("blur", () => {
            window.saveTitleUpdate(el.id, el.innerText.trim());
          });
        });
      }
      ...
      const startCarousel = () => {
        ...
        bulletinInterval = setInterval(() => {
          ...
          setTimeout(() => {
            ...
            bulletinContent.innerHTML = bulletinLines[currentLineIndex];
      ...
      settingsDoc.onSnapshot((doc) => {
        if (doc.exists) {
          ...
          bulletinContent.innerHTML = bulletinLines[0] || "";
          bulletinInput.value = text;
          startCarousel();
        }
      }, (err) => console.warn("公告欄讀取受限:", err));

      // 2. 秘密觸發器：連按 5 下公告欄標題列開啟管理員模式
      let titleClickCount = 0;
      const noticeHeader = document.querySelector(".sidebar-notice .notice-title");
      if (noticeHeader) {
        noticeHeader.addEventListener("click", () => {
          ...
          if (titleClickCount >= 5) {
            isAdmin = true;
            if (editBulletinBtn) {
              editBulletinBtn.style.display = "inline-block";
              editBulletinBtn.style.animation = "blink 1s infinite"; // 閃爍提示
            }
            if (visitorStats) visitorStats.style.display = "inline-block";
            document.body.classList.add("admin-mode-active");
      ...
      const refreshGuestbook = () => {
        guestbookColl.orderBy("time", "desc").limit(20).onSnapshot((snapshot) => {
          const msgs = [];
          snapshot.forEach((doc) => {
            msgs.push({ id: doc.id, ...doc.data() });
          });
          guestbookList.innerHTML = msgs.map(m => `
      ...
      window.saveTitleUpdate = async (id, val) => {
        try {
          if (id === "bulletinContent") {
            await settingsDoc.set({ text: val }, { merge: true });
          } else {
            await configDoc.set({ [id]: val }, { merge: true });
          }
          console.log(`${id} 已自動儲存`);
        } catch (e) { console.error("自動儲存失敗", e); }
      };
    } catch (e) { console.error("管理邏輯載入錯誤", e); }
  }
  // --- 管理邏輯結束 ---
