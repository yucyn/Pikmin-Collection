(function () {
  ...
  /** 全螢幕 fixed 宿主：子元素 absolute 對齊視窗，避免 body flex／動畫 transform 影響 fixed。 */
  ...
  function setNicknameRowMode(mode, displayText) {
    const edit = document.getElementById("v37NicknameEditBlock");
    const bound = document.getElementById("v37NicknameBoundBlock");
    const disp = document.getElementById("v37NicknameDisplay");
  ...
  function setInviteRowMode(mode, displayText) {
    const edit = document.getElementById("v37InviteEditBlock");
    const bound = document.getElementById("v37InviteBoundBlock");
    const disp = document.getElementById("v37InviteDisplay");
    if (!edit || !bound) return;
    if (mode === "bound") {
      edit.hidden = true;
      bound.hidden = false;
  ...
  function updatePanelStatus() {
    ...
    const saveBtn = document.getElementById("v37SaveUnlockBtn");
    ...
    const profileReady = googleLinked && prof !== null;
    const showBoundNickname = profileReady && Boolean(nickCloud);
    const showBoundInvite = profileReady && inviteDigitsAreValid(invCloud);

    if (playerFields) playerFields.hidden = isFirebaseAdmin();

    if (nicknameInput && !showBoundNickname && !nicknameInput.value.trim()) {
      nicknameInput.value = getStoredNicknameForForm();
    }
    if (inviteInput && !showBoundInvite && !inviteInput.value.trim()) {
      const d = getStoredInviteDigitsForForm();
      inviteInput.value = d ? formatInviteDisplay(d) : "";
    }

    setNicknameRowMode(showBoundNickname ? "bound" : "edit", nickCloud);
    setInviteRowMode(showBoundInvite ? "bound" : "edit", formatInviteDisplay(invCloud));

    if (loginBtn) {
      loginBtn.style.display = googleLinked ? "none" : "";
      loginBtn.textContent = "用 Google 登入";
    }
    if (saveBtn) {
      const needSave =
        googleLinked &&
        !isFirebaseAdmin() &&
        profileReady &&
        (!showBoundNickname || !showBoundInvite);
      saveBtn.style.display = needSave ? "" : "none";
    }
    if (logoutBtn) {
    ...
    if (isFirebaseAdmin()) {
      ...
      if (!hasInviteCode()) missing.push("遊戲邀請碼");
      if (!googleLinked) missing.push("Google 登入");
      setStatus(`🌫️ 尚未解鎖，還需要：${missing.join("、")}`);
    }
    createLoginButton();
  }

  async function saveUnlockFromInputsToCloud() {
    const user = getFirebaseUser();
    if (!user || user.isAnonymous) {
      setStatus("🌱 請先使用 Google 登入。");
      return;
    }
    ...
    try {
      await saveUnlockProfileToCloud(user.uid, v.nickname, v.inviteDigits);
      cachedUnlockProfile = { nickname: v.nickname, inviteDigits: v.inviteDigits };
      try {
        localStorage.removeItem(PLAYER_NICKNAME_KEY);
        localStorage.removeItem(INVITE_CODE_KEY);
      } catch (e) {}
      setStatus("🍄 暱稱與邀請碼已儲存並綁定此 Google 帳號。");
      updatePanelStatus();
      refreshCards();
    } catch (error) {
      setStatus(`儲存失敗：${error.message || error}（請確認 Firestore 已啟用且規則允許使用者寫入 ${UNLOCK_DOC_COLLECTION}/{uid}）`);
    }
  }
  ...
  async function clearCloudNickname() {
    const user = getFirebaseUser();
    if (!user || user.isAnonymous) return;
    try {
      await patchUnlockProfileToCloud(user.uid, { nickname: "" });
      if (cachedUnlockProfile) cachedUnlockProfile.nickname = "";
      setStatus("已清除暱稱，請重新填寫並儲存。");
      updatePanelStatus();
      refreshCards();
    } catch (error) {
  ...
  async function clearCloudInvite() {
    const user = getFirebaseUser();
    if (!user || user.isAnonymous) return;
    try {
      await patchUnlockProfileToCloud(user.uid, { inviteDigits: "" });
      if (cachedUnlockProfile) cachedUnlockProfile.inviteDigits = "";
      setStatus("已清除遊戲邀請碼，請重新填寫並儲存。");
      updatePanelStatus();
      refreshCards();
    } catch (error) {
      setStatus(`清除失敗：${error.message || error}`);
  ...
  function createAuthPanel() {
      ...
      <section class="v37-auth-panel" role="dialog" aria-modal="true" aria-label="解鎖蘑菇">
        <div class="v37-auth-header">
          <div>
            ...
            <p class="v37-auth-subtitle">解鎖圖片與 GPS（暱稱與邀請碼綁定 Google 帳號）</p>
          </div>
        ...
        <div id="v37AuthPlayerFields">
          <div class="v37-auth-field">
            ...
            <div id="v37NicknameBoundBlock" class="v37-auth-bound-row" hidden>
              <span class="v37-auth-bound-label">暱稱：</span>
              <strong id="v37NicknameDisplay" class="v37-auth-bound-value"></strong>
              <button type="button" class="v37-auth-cancel-link" id="v37NicknameClearBtn">取消</button>
            </div>
          </div>
          <div class="v37-auth-field">
            ...
            <div id="v37InviteBoundBlock" class="v37-auth-bound-row" hidden>
              <span class="v37-auth-bound-label">遊戲邀請碼：</span>
              <strong id="v37InviteDisplay" class="v37-auth-bound-value"></strong>
              <button type="button" class="v37-auth-cancel-link" id="v37InviteClearBtn">取消</button>
            </div>
          </div>
        </div>
        <div class="v37-auth-actions">
          <button id="v37GoogleLoginBtn" class="v37-auth-primary" type="button">用 Google 登入</button>
          <button id="v37SaveUnlockBtn" class="v37-auth-primary" type="button" style="display:none;">儲存</button>
    ...
    document.getElementById("v37SaveUnlockBtn")?.addEventListener("click", () => saveUnlockFromInputsToCloud());
    document.getElementById("v37LogoutBtn")?.addEventListener("click", signOut);
    document.getElementById("v37NicknameClearBtn")?.addEventListener("click", () => clearCloudNickname());
    document.getElementById("v37InviteClearBtn")?.addEventListener("click", () => clearCloudInvite());
