...
function getCategories() {
  const popular = ["全球","台灣","日本","韓國","香港","澳門","泰國","新加坡","馬來西亞","越南","菲律賓","印度","印尼","蒙古","美國","加拿大","澳洲","紐西蘭","英國","法國","德國","義大利","西班牙","葡萄牙","瑞士","荷蘭","奧地利","希臘","挪威","芬蘭","冰島","土耳其","埃及","沙烏地阿拉伯","墨西哥","巴西","哥倫比亞","阿根廷","智利","馬紹爾群島","杜拜","布拉格","斯洛維尼亞"];
...
function renderPostcards({
  ...
  // V38: 增加渲染守衛，僅在內容、頁碼或模式改變時才重新繪製
  const renderKey = list.map(item => item.id + (item.updatedAt || "") + (item.likeCount || 0)).join("|") + 
                    `_${currentPage}_${totalPages}_${document.body.classList.contains("is-browse-mode")}`;
  
  if (grid.dataset.lastRenderKey === renderKey) return;
  grid.dataset.lastRenderKey = renderKey;
  ...
  list.forEach((item, idx) => {
    ...
    // V38+: 手機版圖片載入重試機制 (解決偶爾發生的圖片讀取失敗)
    image.onerror = () => {
      if (!image.dataset.retryCount) {
        image.dataset.retryCount = "1";
        setTimeout(() => {
          const originalSrc = getDisplayImageUrl(item);
          if (originalSrc) {
            image.src = originalSrc + (originalSrc.includes('?') ? '&' : '?') + 'r=' + Date.now();
          }
        }, 3000);
      }
    };
    image.src = getDisplayImageUrl(item);
...
function renderMapList({ mapList, onSelect, filters }) {
  ...
  list.forEach(item => {
    ...
    thumbnail.src = getDisplayImageUrl(item);
