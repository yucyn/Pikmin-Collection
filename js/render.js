...
function renderPostcards({
  ...
  const renderKey = list.map(item => item.id + (item.likeCount || 0)).join("|") + 
  ...
  list.forEach((item, idx) => {
    ...
    image.onerror = () => {
      if (!image.dataset.retryCount) {
        ...
        setTimeout(() => {
          const originalSrc = getPostcardImageSrc(item);
          if (originalSrc) {
    ...
    image.src = getPostcardImageSrc(item);
...
function renderMapList({ mapList, onSelect, filters }) {
  ...
  list.forEach(item => {
    ...
    thumbnail.src = getPostcardImageSrc(item);
