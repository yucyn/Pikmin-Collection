...
function renderPostcards({
  ...
  list.forEach(item => {
    ...
    const isOwner = typeof isOwnedByCurrentUser === "function" ? isOwnedByCurrentUser(item) : false;
    
    // 權限定義：管理員可管所有，登入用戶可管自己
    ...
    if (typeof onEditClick === "function" && !isBrowseMode && canManage) {
    ...
    // 預覽模式下不顯示更多按鈕；其餘模式（包含未登入）均顯示以供分享
    if (!isBrowseMode) {
