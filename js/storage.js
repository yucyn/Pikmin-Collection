...
window.getDisplayImageUrl = function(card) {
  if (!card) return "";
  let url = card.imageUrl || card.imageBase64 || card.image || "";
  if (!url || url.startsWith("data:")) return url;

  // 移除既有的 v= 參數，避免版本停留在舊值
  if (url.includes("v=")) {
    url = url.split(/[?&]v=/)[0];
  }

  // 取得版本號：優先用 updatedAt，若無則用 createdAt，最後用 Date.now()
  let version = Date.now();
  if (card.updatedAt) {
    // 確保版本號是數字或乾淨的字串，避免特殊字元問題
    const date = new Date(card.updatedAt);
    version = isNaN(date.getTime()) ? card.updatedAt : date.getTime();
  } else if (card.createdAt) {
    const date = new Date(card.createdAt);
    version = isNaN(date.getTime()) ? card.createdAt : date.getTime();
  }
...
window.getPostcardImageSrc = function(card) {
  return window.getDisplayImageUrl(card);
};
...
window.uploadImageToStorage = async function(base64Data) {
  ...
  try {
    ...
    // 方案 C: 加入 Cache-Control 元資料，強制快取 1 年 (immutable)
    const metadata = {
      cacheControl: 'public,max-age=31536000,immutable'
    };
    // 使用 putString 上傳 Data URL 並帶入 metadata
    const snapshot = await storageRef.putString(base64Data, 'data_url', metadata);
...
window.isOwnedByCurrentUser = function(item) {
  if (window.isCurrentUserAdmin()) return true;

  // V39: 陌生使用者（匿名）只能上傳，不能編輯
  ...
  return Boolean(item && item.ownerId && item.ownerId === currentUser.uid);
};
...
window.initializeFirebaseStorage = function(onChange) {
  // V38: 全域初始化旗標，防止重複建立監聽器導致效能下降
  if (window.isPostcardListenerStarted) {
    console.log("Firebase 監聽器已在運行中，跳過重複初始化。");
    return;
  }
  window.isPostcardListenerStarted = true;
  ...
  // V38+: 啟用離線快取 (Persistence)，大幅優化手機版首次載入速度與不穩網速下的體驗
  if (!window.db._persistenceStarted) {
    window.db._persistenceStarted = true;
  ...
  const startSnapshot = () => {
    if (unsubscribe) return; // 已在監聽中，避免重複開啟
    
    unsubscribe = window.db.collection(collectionName).onSnapshot(snapshot => {
      // V38: 增加資料異動判定，避免無意義的頻繁渲染
      const newData = snapshot.docs.map(doc => normalizePostcard(doc.id, doc.data()));
      newData.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
      
      window.postcardsCache = newData;
      onChange();
    }, error => {
      console.warn("❌ Firestore 讀取失敗，將於 5 秒後自動重試:", error);
      setTimeout(() => {
        unsubscribe = null;
        startSnapshot();
      }, 5000);
...
window.updatePostcard = async function(id, data) {
  ...
  try {
    ...
    // V39: 如果編輯中有更換圖片 (Base64)，先上傳至 Storage
    if (updates.image && updates.image.startsWith("data:")) {
      const newUrl = await window.uploadImageToStorage(updates.image);
      updates.imageUrl = newUrl;
      // 確保移除 Base64，避免存入 Firestore 導致 document 過大
